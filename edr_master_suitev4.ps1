...
function Invoke-Revert {
    ...
    # -- 3. FIREWALL -- re-enable --
    Write-Detail "Re-enabling Windows Firewall..."
    $fw = netsh advfirewall set allprofiles state on 2>&1
    Write-Evidence "Firewall result: $fw"
    if ($fw -match "Ok") { Revert-Log "Windows Firewall RE-ENABLED (all profiles)" }
    else { Revert-Fail "Firewall: $fw" }
