...
function Test-Recon {
    ...
    @{
        ...
        Last_Boot     = [Management.ManagementDateTimeConverter]::ToDateTime($os.LastBootUpTime)
...
"@
    ...
    Write-Host "  |  MODULE REPORT 2 -- CREDENTIAL DUMPING            |" -ForegroundColor Cyan
    Write-Host "  +====================================================+" -ForegroundColor Cyan
    Write-Host ""
...
function Test-Ransomware {
    ...
    # Create victim files ON DESKTOP in visible folder
    Write-Detail "Creating victim files on Desktop..."
    $desktopPath = [Environment]::GetFolderPath("Desktop")
    $testDir = "$desktopPath\ENCRYPTED_FILES"
    New-Item -ItemType Directory -Force -Path $testDir | Out-Null
    
    Write-Host ""
    Write-Host "  📁 Files location: Desktop\ENCRYPTED_FILES" -ForegroundColor Yellow
    ...
    foreach ($f in $files.GetEnumerator()) {
        ...
        $size = (Get-Item "$testDir\$($f.Key)").Length
        Write-Evidence "✓ Created: $($f.Key) ($size bytes)"
    }
    ...
    # Ransom note - in encrypted folder on Desktop
    Write-Host ""
...
"@
    ...
    Write-Evidence "📄 Ransom note created in: $testDir"
    Write-OK "Ransom note placed in encrypted folder"
...
"@ -ErrorAction SilentlyContinue
    ...
    # Summary showing Desktop location
    Write-Host ""
    Write-Host "╔════════════════════════════════════════════════════════════════╗" -ForegroundColor Green
    Write-Host "║              ✅ RANSOMWARE SIMULATION COMPLETE ✅              ║" -ForegroundColor Green
    Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor Green
    Write-Host ""
    Write-Host "  📁 ENCRYPTED FILES LOCATION:" -ForegroundColor Cyan
    Write-Host "     Desktop\ENCRYPTED_FILES" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "  Full Path: $testDir" -ForegroundColor White
    Write-Host ""
    Write-Host "  Files encrypted:" -ForegroundColor White
    Get-ChildItem $testDir -ErrorAction SilentlyContinue | ForEach-Object {
...
function Test-SandboxEvasion {
    ...
    $checks = @(
        ...
        @{ Name="Uptime < 10 min"; Check={
            ...
            $up = (Get-Date) - [Management.ManagementDateTimeConverter]::ToDateTime($osInfo.LastBootUpTime)
